...
{
    public partial class OtobusBileti : UserControl
    {
        ...
        public OtobusBileti(int userId) : this()
        {
            _userId = userId;
        }

        private void OtobusBileti_Load(object sender, EventArgs e)
        {
            LoadTerminals();
            SetupDatePicker();
        ...
        private void LoadTerminals()
        {
            try
            {
                var dt = TicketService.GetTerminalsByType(Constants.VEHICLE_TYPE_BUS);
            ...
            {
                MessageHelper.ShowError("Otogar verileri yüklenirken hata: " + ex.Message);
            }
        }
        ...
        private void btnAra_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageHelper.ShowWarning("Lütfen kalkış ve varış otogarlarını seçin.");
                return;
            }
            ...
            {
                MessageHelper.ShowWarning("Kalkış ve varış otogarları aynı olamaz.");
                return;
            }

            // Şehir içi yolculuk kontrolü
            if (TicketService.IsSameCity(departure, arrival))
            {
                MessageHelper.ShowWarning("Şehir içi yolculuk yapılamaz. Lütfen farklı şehirler seçin.");
                return;
            }
        ...
        private void SearchTrips()
        {
            try
            {
                ...
                var dt = TicketService.SearchTrips(Constants.VEHICLE_TYPE_BUS, selectedDate, departure, arrival);

                if (dt.Rows.Count == 0)
                {
                    MessageHelper.ShowInfo($"{selectedDate:dd.MM.yyyy} tarihinde bu güzergahta otobüs seferi bulunamadı.");
                }
                else
                {
            ...
            {
                MessageHelper.ShowError("Otobüs seferi aranırken hata: " + ex.Message);
            }
        }
        ...
        private void LoadSeats()
        {
            ...
            for (int i = 1; i <= Constants.SEAT_COUNT_BUS; i++)
            {
                bool isOccupied = _occupiedSeats.Contains(i);
                var btnSeat = ThemeHelper.CreateSeatButton(i, isOccupied);
        ...
        private void PurchaseTicket()
        {
            ...
            if (success)
            {
                MessageHelper.ShowSuccess($"🎉 Otobüs bileti başarıyla satın alındı!\n\nKoltuk No: {_selectedSeatNo}\nFiyat: {_ticketPrice:N2} ₺");
                LoadSeats();
            }
            else
            {
